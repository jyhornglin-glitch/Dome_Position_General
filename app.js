...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function drawLocalGridPath(targetSvg = null, targetIdx = null) {
    ...
    // Always include Stage B center (-7.5, 37.5) in the visible map area
    if (!homeCoord.isText) {
      const stageB_dx_rel = -7.5 - homeCoord.x;
    ...
    if (!homeCoord.isText) {
      ...
      const bg_x1_rel = -10.5 - homeCoord.x;
      ...
      // Stage B Circular Background: Col = -7.5, Row = 37.5, Radius = 8.7 (outermost step)
      const stageB_dx_rel = -7.5 - homeCoord.x;
      const stageB_dy_rel = 37.5 - homeCoord.y;
      ...
      // 1. Draw Runway Central Rectangle: Col = -10.5 to -4.5, Row = 32.5 to 42.5
      const rect_x1_rel = -10.5 - homeCoord.x;
      ...
      sides.forEach(side => {
        for (let i = 0; i <= 6; i++) {
          ...
          const col_top = -7.5 + side * W_i - homeCoord.x;
          const col_mid = -7.5 + side * R_i - homeCoord.x;
          const col_bottom = -7.5 + side * W_i - homeCoord.x;
      ...
      // 3. Draw radial stairs/steps on Stage B: radiating from center (-7.5, 37.5) on BOTH sides
      ...
      // 4. Draw Faint text label "乙舞台" centered at (-7.5, 37.5)
