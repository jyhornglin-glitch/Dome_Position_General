...
def create_manual():
    ...
    intro_text = (
        ...
        "演繹人員只需輸入自己所在的「起點座標」，系統即會自動繪製並計算在 7 大不同演繹隊形 "
        "（基本、圓形、行願、靜思、有法船、無法船、四弘誓願）中的相對位置、行進方向、步數以及歌詞段落。 "
    ...
    p1 = (
        ...
        "1. 起點搜尋與聯想清單：\n"
        "   - 在畫面上方的搜尋框中輸入您的起點座標（例如：「1-50」或「7-37」）。\n"
    ...
    p2 = (
        ...
        "2. 指引線路徑：黃色粗線與箭頭為目前步驟的行進動線。勾選「顯示完整 7 點軌跡」後，會同時以各步驟的專屬色彩顯示完整的連續移動路線。"
    )
    pdf.set_left_margin(20)
    ...
    p3 = (
        ...
        "2. 隊形步驟與頁籤切換按鈕：\n"
        "   - 「上一個」與「下一個」按鈕：位於地圖上方，用於在 7 個演繹隊形步驟間切換。\n"
    ...
    p4 = (
        ...
        "1. 跑位引導步驟：\n"
        "   - 在「跑位引導」頁籤中，系統以步驟清單詳細列出 7 個演繹步驟的行進指引。\n"
        "   - 每一步驟包含：目標絕對座標、行進方向提示（如：向右後 3.5 步）、直線距離步數，以及對應的歌詞OS段落。\n\n"
        "2. 隊形詳情圖卡：\n"
        "   - 在「隊形詳情」頁籤中，列出了 7 大隊形點位的專屬圖卡。\n"
    ...
    p5 = (
        "1. 查看與下載所有圖：\n"
        "   - 點擊「查看與下載所有圖」按鈕，會開啟彈出視窗預覽 7 個演繹隊形的縮圖。\n"
        "2. 高解析度 PDF 下載：\n"
    ...
    rect_y = pdf.get_y()
    pdf.rect(15, rect_y, 180, 13, 'DF')
    pdf.set_y(rect_y + 1.5)
    pdf.cell(5) # spacer
